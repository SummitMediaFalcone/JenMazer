...
written to look for them, so once they're in place it just works.

---

## 1. Her headshot → `public\images\`
...
## 3. Party / food photos → `public\images\parties\`
...
Descriptive names, lowercase, hyphens — never `IMG_4471.jpg`. Google Images is
a real traffic source for artists, and the filename is part of how work gets
found there.

## Then what

Tell me they're in and I'll point the site at them (one line per image) and
crop/frame them properly. Until then the site runs on placeholder graphics so
it still looks right — **if you see placeholder boxes, that's expected, not
broken.**

## A note on the cupcake photos

Two of them have a rehab facility brochure and a plastic clamshell in frame.
They still work — I'll crop tight to the cupcakes. But if she has cleaner
shots of the same work, those would be better. Same for anything shot on a
garage floor: good light and a plain surface doubles what these photos are
worth.
